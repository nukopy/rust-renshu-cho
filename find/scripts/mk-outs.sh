...
echo "Creating expected outputs in \"$OUT_DIR\""
echo "Checking \"$IN_DIR\" exists..."
if [[ ! -d "$IN_DIR" ]]; then
...
echo "Creating \"$OUT_DIR\" if it doesn't exist..."
if [[ ! -d "$OUT_DIR" ]]; then
...
echo "Removing old outputs forcibly..."
rm -f $OUT_DIR/*.txt
echo "ok"

echo "Creating new expected outputs..."
